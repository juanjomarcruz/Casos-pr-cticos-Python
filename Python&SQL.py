...
sql_query_4="SELECT AVG(actual_departure - scheduled_departure) as Average_Departure_Delay FROM flights"
...
sql_query_7="SELECT aircraft_code, departure_airport, COUNT(flight_id) as Total_flights_plus30min_delay FROM flights WHERE (actual_departure - scheduled_departure) > (30*'1 min'::interval) GROUP BY aircraft_code,departure_airport"
...
sql_query_10="SELECT aircraft_code, AVG(actual_departure-scheduled_departure) as average_departure_delay FROM flights WHERE aircraft_code IN (SELECT aircraft_code FROM flights WHERE Status='Delayed' GROUP BY aircraft_code) GROUP BY aircraft_code"
...
sql_query_13="SELECT aircraft_code, COUNT(flight_id) as Total_flights FROM flights WHERE aircraft_code IN (SELECT aircraft_code FROM flights WHERE Status='Delayed' GROUP BY aircraft_code) GROUP BY aircraft_code"
...
sql_query_15="SELECT aircraft_code, AVG(actual_departure-scheduled_departure) as average_departure_delay FROM flights WHERE aircraft_code IN (SELECT aircraft_code FROM flights WHERE Status='Delayed' GROUP BY aircraft_code) GROUP BY aircraft_code"
cursor=conn.cursor()
